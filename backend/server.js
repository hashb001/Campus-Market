...
// POST create listing (with image upload)
app.post('/api/listings', upload.single('image'), async (req, res) => {
    try {
        const { productName, itemInfo, sellerName, sellerNumber, category, listingType } = req.body;
...
// POST admin contact message (stored in session, no DB needed)
app.post('/api/contact', (req, res) => {
    const { email, message } = req.body;
    if (!email || !message) return res.status(400).json({ error: 'Email and message required' });

    // Store in session as simple confirmation
